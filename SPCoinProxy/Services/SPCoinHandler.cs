...
namespace SPCoinProxy.Services
{
    public class SPCoinHandler : ISPCoinHandler
    {
        ...
        {
            BalanceRequestPacket request = new BalanceRequestPacket(uuid);
            await s.WriteAsync(request);
            BalanceResponsePacket response = await s.ReadAsync<BalanceResponsePacket>();
        ...
        public Task<bool> IncreaseUserBalance(Guid uuid, string reason, int value, CancellationToken cancellationToken) => RunClient(async s =>
        {
            ...
            BalanceSetupRequestPacket request = new BalanceSetupRequestPacket(uuid, reason, value);
            await s.WriteAsync(request);
            try
            {
        ...
        {
            using SPCoinSocket socket = new SPCoinSocket(url, cancellationToken);
        ...
        private class SPCoinSocket : IDisposable
        {
            ...
            {
                T packet;
                JObject json = await ReadJsonAsync();

                string? _type = json["type"]?.Value<string>();
                string _key;
